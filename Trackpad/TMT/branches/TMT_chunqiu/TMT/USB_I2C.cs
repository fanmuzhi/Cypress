...
namespace CypressSemiconductor.ChinaManufacturingTest.TrackpadModuleTester
{
    public class USB_I2C
    {
        ...
        //Read and Write I2C
        public void WriteReg(byte address, byte command, int delay_time_ms)
        {
           
            pp.I2C_SendData(deviceAddress, new byte[] { address, command }, out lastError);
            System.Threading.Thread.Sleep(delay_time_ms);               
              
        }

        public byte[] ReadReg(byte address, int nToRead, int delay_time_ms)
        {
           
            byte[] data;
            object dataIn;
            pp.I2C_SendData(deviceAddress, new byte[] { address }, out lastError);
            System.Threading.Thread.Sleep(delay_time_ms);
            pp.I2C_ReadData(deviceAddress, nToRead, out dataIn, out lastError);
           // dataIn = pp.I2C_ReadDataFromReg(deviceAddress, address, nToRead, out dataIn, out lastError);               
            data = dataIn as byte[]; 
            return data;
        }
