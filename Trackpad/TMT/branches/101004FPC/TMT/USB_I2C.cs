...
namespace CypressSemiconductor.ChinaManufacturingTest.TrackpadModuleTester
{
    public class USB_I2C
    {
        ...
        public byte[] ReadWrite(byte[] commands, byte nToRead, int delay_time_ms)  //This is for 10100400
        {
            byte repeat = 0;
            bool success = false;
            byte[] data;
            ...
            do
            {
                ...
                data = dataIn as byte[];
                if (data[0] == commands[1])
                { success = true; }

                repeat++;

            } while (!success && repeat < 100);

            if (!success)
            {
                Log.error("Fail to excute command: " + commands.ToString());
            }
