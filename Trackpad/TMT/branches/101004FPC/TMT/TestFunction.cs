    ...
    public class TestFunction
    {
        ...
        private void I2C_Read_RawX_Test()
        {
            ...
            for (int i = 0; i < RAW_DATA_READS; i++)
            {
                ...
                //OnTestMessage(new TestMessageEventArgs("RawCountX: " + datain_RawCountX[i].ToString()));
        ...
        private void I2C_Enter_TestMode()
        {
            ...
            if (dut.SerailNumber.Substring(0, 8) == "10100400")
            {
                
                byte[] commands = new byte[4] { 0x00, 0xE0, 0x0A, 0x0F };// only for 10100400
                //for (int i = 0; i < 100; i++)
                //{
                //    bridge.Write(commands);
                //    System.Threading.Thread.Sleep(10);
                //}
                byte[] datain = bridge.ReadWrite(commands, Convert.ToByte(15), 10);
