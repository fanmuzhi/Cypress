...
namespace CypressSemiconductor.ChinaManufacturingTest.TrackpadModuleTester
{
    ...
    public class TestFunction
    {
        ...
        private void I2C_ST_MTG_Signal_Test()
        {
            ...
            byte[] datain3 = bridge.ReadWrite(0x08,20,I2CReadDelay);


            for (int i = 0; i < 16; i++)
            {
                int temp = datain3[i+3];
                dut.Signal_Single.Add(temp);
        ...
        public void I2C_ST_MTG_Signal_Check()
        {
            ...
            ////X sensor test
            //if (Math.Abs(dut.Signal[0] - dut.Signal[7]) > (dut.Signal[0] + dut.Signal[7]) / cfg.X_diff || Math.Abs(dut.Signal[1] - dut.Signal[6]) > (dut.Signal[1] + dut.Signal[6]) / cfg.X_diff || Math.Abs(dut.Signal[2] - dut.Signal[5]) > (dut.Signal[2] + dut.Signal[5]) / cfg.X_diff || dut.Signal[4] - dut.Signal[3] < dut.Signal[3] / 5)
            //{
            //    dut.ErrorCode = ErrorCode.ERROR_FINGER_MOVE;
            //}
            //if ((dut.Signal[2] - dut.Signal[3]) < (dut.Signal[2] + dut.Signal[3])/cfg.X_45_diff)
            //{
            //    dut.ErrorCode = ErrorCode.ERROR_FINGER_MOVE;
            //}
                       
            ////Y sensor test
            //if (Math.Min(dut.Signal[8], dut.Signal[10]) < dut.Signal[9] || Math.Min(dut.Signal[8], dut.Signal[11]) < dut.Signal[9])
            //{
            //    dut.ErrorCode = ErrorCode.ERROR_FINGER_MOVE;
            //}
            
            //if (order[6] != 1 || order[5] != 2 || order[7] != 3 || order[4] != 4)
            //{
            //    dut.ErrorCode = ErrorCode.ERROR_FINGER_MOVE;
            //}
