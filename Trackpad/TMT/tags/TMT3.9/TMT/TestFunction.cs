...
namespace CypressSemiconductor.ChinaManufacturingTest.TrackpadModuleTester
{
    ...
    public class TestFunction
    {
        ...
        private int RAW_DATA_READS_DELAY=0; //20
        ...
        public void SwitchTest()
        {
            ...
            while (!switchDetected)
            {
                ReadPosition();

                if ((dut.LeftButtonStatus & 0x04) == 0x04 || (dut.LeftButtonStatus & 0x01) == 0x01)
                {
                    switchDetected = true;
                }
            }
        ...
        private void I2C_Read_RawX_Test()
        {
            ...
            for (int i = 0; i < RAW_DATA_READS; i++)
            {
                ...
                System.Threading.Thread.Sleep(RAW_DATA_READS_DELAY);
            }
        ...
        private void I2C_APA_Read_RawCount_Test()
        {
            ...
            for (int i = 0; i < RAW_DATA_READS; i++)
            {
                ...
                {
                    templist.Add(bridge.ReadWrite(TestCommand.Read_RawCount_X, row, (byte)(3 + 2 * NUM_COLS), DelayTime.I2C_MS / 2));
                    //datain_APA_RawCount_temp.Add(templist);                    
        ...
        private void I2C_Enter_TestMode()
        {
            ...
            else if (dut.SerailNumber.Substring(0, 8) == "11400200")
            {
                byte[] commands = new byte[3] { 0x30, 0x0A, 0x0F };
                bridge.Write(commands);
                System.Threading.Thread.Sleep(DelayTime.I2C_Enter_TestMode);
            }
            else if (dut.SerailNumber.Substring(0, 8) == "01200700" || dut.SerailNumber.Substring(0, 8) == "01200900")
            {
        ...
        public void Cal_APA_RawCount_Noise()
        {
            ...
            for (int i = 0; i < RAW_DATA_READS; i++)
            {
                for (byte row = 0; row < NUM_ROWS; row++)
                {
                    List<byte[]> tempList = datain_APA_RawCount[i];
                    byte[] datain = tempList[row];
        ...
        public void Check_APA_LocalIDAC_TotalIDAC()
        {
            ...
            //// check 11900100 total Idac
            if (dut.SerailNumber.Substring(0, 8) == "11900100")
            {
        ...
        public void Check_Other_IDAC()
        {
            ...
            foreach (int idacValue in dut.Global_IDAC.GetRange(0, NUM_COLS + NUM_ROWS))
            {
                //newIDACrow[idac_element + 1] = temp;
                //idac_element++;
