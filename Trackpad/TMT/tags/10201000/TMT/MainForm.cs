    ...
    public partial class MainForm : Form
    {
        ...
        public class Mode
        {
            ...
            public Mode(byte mode)
            {
                ...
                {
                    UISize = new System.Drawing.Size(1024, 700);
                    SignalTest = false;
                    PostionTest = true;
                ...
                {
                    UISize = new System.Drawing.Size(1024, 184);
                    SignalTest = false;
                    PostionTest = false;
        ...
        private bool validateSerialNumber(string sn)
        {
            ...
            Regex r = new Regex("^[01]([0][0-9]|[1][0-9])([0][0-9][0-9])[0][01][0-9][1-9]([0][1-9]|[1][0-5])([0-4][0-9]|[5][0-3])[0-6][a-zA-Z0-9][a-zA-Z0-9][a-zA-Z0-9]([1-8]$|$)");
        ...
        private void testProcess()
        {
            ...
            try
            {
                ...
                System.Threading.Thread.Sleep(500);
                testFunction.ExitBootloader();
                if (!testFunction.I2C_Check_RawCount_Value())
                {
        ...
        private void PositionPanelPaint(object sender, PaintEventArgs e)
        {
            ...
            Pen penB = new Pen(pointB, 5);
            SolidBrush pointC = new SolidBrush(Color.Red);
            Pen penC = new Pen(pointC, 15);
            int finger1X;
            int finger1Y;
            int radiusTemp = Convert.ToInt32(radius / positionPanelScale);
            //testFunction.fingerY = Convert.ToInt32(resolutionY.Text) - testFunction.fingerY;

            if (testFunction.fingerX < 3000 && testFunction.fingerY < 3000 && testFunction.fingerX > 0 && testFunction.fingerY > 0 && testFunction.currentTarget<5)
            {
                try
                {
                ...
                try
                {
                    ...
                    if (Math.Pow((finger2X - finger1X) * positionPanelScale, 2) + Math.Pow((finger2Y - finger1Y) * positionPanelScale, 2) < Math.Pow(radius, 2))
                    {
                        ...
                        if (fingerCount > 3)
                        {
                            ...
                            if (testFunction.currentTarget < testFunction.targetPoint.Count())  // && textBoxSwitchL.BackColor == Color.Green && textBoxSwitchR.BackColor == Color.Green)
                            {
                    ...
                    if (testFunction.currentTarget < 5)
                    {
            ...
            else if(testFunction.currentTarget < 5)
            {
                int jj = 0;
                Point[] fingerpositions = new Point[testFunction.fingerPoint.Count];
                foreach (Point fingertemp in fingerpositions)
            ...
            try
            {
                ...
                //    }
                //}
                if ((testFunction.currentTarget > 4) && textBoxSwitchR.BackColor == Color.Green)// && textBoxSwitchR.BackColor == Color.Green)
                {
                    testFunction.readPositionStop = false;
                    tabControlMain.SelectedIndex = 0;
                }
