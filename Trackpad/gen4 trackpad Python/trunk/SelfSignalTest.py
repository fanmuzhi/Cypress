...
returnData={}
returnData['TestItem']='SelfSignal'
...
def SelfSignalTest():
    ...
    if ReCalibration:
        WaitForComplet=True
        bridge.WriteI2CData(0x24, 0x00, SelfSignalCommands.ReCalMulCap)
        ...
        WaitForComplet=True
        bridge.WriteI2CData(0x24, 0x00, SelfSignalCommands.ReCalSelCap)
        while (WaitForComplet):        
            sleep(0.2)
            datain=bridge.ReadI2CData(0x24, 0x00, 3)
            if (datain[2]&0x40==0x40):
    ...
    WaitForComplet=True
    bridge.WriteI2CData(0x24, 0x00, SelfSignalCommands.InitBaseline)
    while WaitForComplet:
        sleep(0.1)
        datain=bridge.ReadI2CData(0x24, 0x00, 3)        
    ...
    for i in range(readCount):        
        ...
        WaitForComplet=True
        bridge.WriteI2CData(0x24, 0x00, SelfSignalCommands.FullPanelScan)
        while WaitForComplet:
            sleep(0.1)
            datain=bridge.ReadI2CData(0x24, 0x00, 3)        
            if (datain[2]&0x80==0x80) or (datain[2]&0x40==0x40):
        ...
        WaitForComplet=True
        bridge.WriteI2CData(0x24, 0x00, SelfSignalCommands.GetSensorFullPanelScan)
        while WaitForComplet:
            sleep(0.1)
            datain=bridge.ReadI2CData(0x24, 0x00, 3)        
            if (datain[2]&0x80==0x80) or (datain[2]&0x40==0x40):
        ...
        SelfSignalTemp.append(datain[8:(8+TrackpadColume+TrackpadRow)])
        
    
    SelfSignalTemp=np.array(SelfSignalTemp,dtype='b')
    SelfSignal=SelfSignalTemp.mean(axis=0)
    #print SelfSignal
    
    
    for dataTemp in SelfSignal:
        returnData['Data'].append(int(dataTemp))
    
    for dataTemp in returnData['Data']:        
        if  dataTemp<SelfSignalLimits.SelfSignalLow:
            returnData['ErrorMessage']="Low self Finger Signal"
            returnData['TestResult']='Fail'
            returnData['ErrorCode']=0x64
            powerOff()
            closePort()    
            jsonReturn=json.dumps(returnData, True)
            print jsonReturn
            return jsonReturn   
        if  dataTemp>SelfSignalLimits.SelfSignalHigh:
            returnData['ErrorMessage']="Hig self Finger Signal"
            returnData['TestResult']='Fail'
            returnData['ErrorCode']=0x65
            powerOff()
            closePort()    
            jsonReturn=json.dumps(returnData, True)
            print jsonReturn
...
if __name__=="__main__":      
    ...
    configTest=configFile_data.get_item("Self-cap Signal",configPath)    
    SelfSignalLimits.SelfSignalHigh=configTest.Max
    SelfSignalLimits.SelfSignalLow=configTest.Min
    ...
    SelfSignalCommands.GetSensorFullPanelScan[5]=(TrackpadColume+TrackpadRow)/256
    SelfSignalCommands.GetSensorFullPanelScan[6]=(TrackpadColume+TrackpadRow)%256
