...
#         return jsonReturn
    ...
    if ReCalibration:
        WaitForComplet=True
        bridge.WriteI2CData(0x24, 0x00, RawDataCommands.ReCalMulCap)
        ...
        WaitForComplet=True
        bridge.WriteI2CData(0x24, 0x00, RawDataCommands.ReCalSelCap)
        while (WaitForComplet):        
            sleep(0.2)
            datain=bridge.ReadI2CData(0x24, 0x00, 3)
            if (datain[2]&0x40==0x40):
    ...
    WaitForComplet=True
    bridge.WriteI2CData(0x24, 0x00, RawDataCommands.InitBaseline)
    while WaitForComplet:
        sleep(0.1)
        datain=bridge.ReadI2CData(0x24, 0x00, 3)        
    ...
    for i in range(readCount):        
        ...
        WaitForComplet=True
        bridge.WriteI2CData(0x24, 0x00, RawDataCommands.FullPanelScan)
        while WaitForComplet:
            sleep(0.1)
            datain=bridge.ReadI2CData(0x24, 0x00, 3)        
            if (datain[2]&0x80==0x80) or (datain[2]&0x40==0x40):
        ...
        WaitForComplet=True
        bridge.WriteI2CData(0x24, 0x00, RawDataCommands.GetSensorFullPanelScan)
        while WaitForComplet:
            sleep(0.1)
            datain=bridge.ReadI2CData(0x24, 0x00, 3)        
            if (datain[2]&0x80==0x80) or (datain[2]&0x40==0x40):
        ...
        rawDataTemp.append(datain[8:(8+TrackpadColume*TrackpadRow)])
        
    
    rawDataTemp=np.array(rawDataTemp,dtype='b')
    rawData=rawDataTemp.mean(axis=0)
    #print rawData
    
    
    for dataTemp in rawData:
        returnData['Data'].append(int(dataTemp))
    
    for dataTemp in returnData['Data']:        
        if  dataTemp>RawDataLimits.RawDataHigh:
            returnData['ErrorMessage']="High Raw Data"
            returnData['TestResult']='Fail'
    ...
    for dataTemp in returnData['Data']:        
        if  dataTemp<RawDataLimits.RawDataLow:
            returnData['ErrorMessage']="Low Raw Data"
            returnData['TestResult']='Fail'
...
if __name__=="__main__": 
    ...
    configTest=configFile_data.get_item("Raw Data",configPath)    
    RawDataLimits.RawDataHigh=configTest.Max
    RawDataLimits.RawDataLow=configTest.Min
    readCount=configTest.Samples 
    RawDataCommands.GetSensorFullPanelScan[5]=TrackpadColume*TrackpadRow/256
    RawDataCommands.GetSensorFullPanelScan[6]=TrackpadColume*TrackpadRow%256
