...
returnData={}
returnData['TestItem']='SelfBaseline'
...
class SelfBaselineCommands:
    InitSelfBaseline   =[0xA0, 0x00, 0x0A, 0x0F]
    FullPanelScan   =[0xA0, 0x00, 0x0B, 0x00, 0x00, 0x00, 0x00, 0x00]
    GetSensorFullPanelScan     =[0xA0, 0x00, 0x0C, 0x00, 0x00, 0x01, 0xE5, 0x04]
    ReCalMulCap   =[0xA0, 0x00, 0x09, 0x00, 0x00, 0x00]
    ReCalSelCap   =[0xA0, 0x00, 0x09, 0x02, 0x00, 0x00]

class SelfBaselineLimits:   
    SelfBaselineHigh              =50
    SelfBaselineLow              =-50
...
#         return jsonReturn
    ...
    if ReCalibration:
        WaitForComplet=True
        bridge.WriteI2CData(0x24, 0x00, SelfBaselineCommands.ReCalMulCap)
        ...
        WaitForComplet=True
        bridge.WriteI2CData(0x24, 0x00, SelfBaselineCommands.ReCalSelCap)
        while (WaitForComplet):        
            sleep(0.2)
            datain=bridge.ReadI2CData(0x24, 0x00, 3)
            if (datain[2]&0x40==0x40):
    ...
    #Initialize SelfBaselines
    WaitForComplet=True
    bridge.WriteI2CData(0x24, 0x00, SelfBaselineCommands.InitSelfBaseline)
    while WaitForComplet:
        sleep(0.1)
        datain=bridge.ReadI2CData(0x24, 0x00, 3)        
    ...
    for i in range(readCount):        
        ...
        WaitForComplet=True
        bridge.WriteI2CData(0x24, 0x00, SelfBaselineCommands.FullPanelScan)
        while WaitForComplet:
            sleep(0.1)
            datain=bridge.ReadI2CData(0x24, 0x00, 3)        
            if (datain[2]&0x80==0x80) or (datain[2]&0x40==0x40):
        ...
        WaitForComplet=True
        bridge.WriteI2CData(0x24, 0x00, SelfBaselineCommands.GetSensorFullPanelScan)
        while WaitForComplet:
            sleep(0.1)
            datain=bridge.ReadI2CData(0x24, 0x00, 3)        
            if (datain[2]&0x80==0x80) or (datain[2]&0x40==0x40):
        ...
        SelfBaselineTemp.append(datain[8:(8+TrackpadColume+TrackpadRow)])
        
    
    SelfBaselineTemp=np.array(SelfBaselineTemp,dtype='b')
    SelfBaseline=SelfBaselineTemp.mean(axis=0)
    #print SelfBaseline
    
    
    for dataTemp in SelfBaseline:
        returnData['Data'].append(int(dataTemp))
    
    for dataTemp in returnData['Data']:        
        if  dataTemp<SelfBaselineLimits.SelfBaselineLow:
            returnData['ErrorMessage']="Low SelfBaseline"
            returnData['TestResult']='Fail'
            returnData['ErrorCode']=0x66
            powerOff()
            closePort()    
            jsonReturn=json.dumps(returnData, True)
            print jsonReturn
            return jsonReturn   
        if  dataTemp>SelfBaselineLimits.SelfBaselineHigh:
            returnData['ErrorMessage']="High SelfBaseline"
            returnData['TestResult']='Fail'
            returnData['ErrorCode']=0x67
            powerOff()
            closePort()    
            jsonReturn=json.dumps(returnData, True)
            print jsonReturn
...
if __name__=="__main__":    
    ...
    configTest=configFile_data.get_item("Self-cap Baseline",configPath)    
    SelfBaselineLimits.SelfBaselineHigh=configTest.Max
    SelfBaselineLimits.SelfBaselineLow=configTest.Min
    readCount=configTest.Samples 
    SelfBaselineCommands.GetSensorFullPanelScan[5]=(TrackpadColume+TrackpadRow)/256
    SelfBaselineCommands.GetSensorFullPanelScan[6]=(TrackpadColume+TrackpadRow)%256
