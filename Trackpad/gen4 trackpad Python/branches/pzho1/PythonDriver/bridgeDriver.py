...
#                    self._writeMessage('ClosePort', msg, self.kWarning)
    ...
    def ReadI2CData(self, offset=0, numBytes=61):
        ...
        try:
            ...
            if numBytes <= maxPacketSize:
                return self._readSmallData(offset, numBytes)
            else:
                ...
                for packetNum in range(0, numPackets):
                    ...
                    smallPacket = self._readSmallData(offset, bytesToRead)
    ...
    #                             _readSmallData
    #---------------------------------------------------------------------------
    def _readSmallData(self, offset=0, numBytes=61):
        ''' This "private" method reads small I2C packets (up to 61 bytes). '''
    
        # Set the offset of the read
        self.WriteI2CData(offset, None)

        data = None

        try:
            # Read the data
            result = self._bridge.I2C_ReadData(self.deviceAddress, numBytes)
            ...
            if (self._OperationSucceeded(status) == False):
                ...
                msg += '    Device Address: ' + str(self.deviceAddress) + '\n'
    ...
    def WriteI2CData(self, offset=0, data=None):
        ...
        try:
            ...
            # Send the data
            result = self._bridge.I2C_SendData(self.deviceAddress, sendData)
            status = result[1]
            lastError = result[0]
    ...
    def EnterInterruptMode(self, bulkBufferSize = 175):
        ...
        try:
            ...
            inData = [0x02,  0x00,  0x94, self.deviceAddress,  0x00,  bulkSizeLo, bulkSizeHi]
    ...
    def ExitBootloaderMode(self):
        ...
        try:
            while True:    
                ...
                # Write the data            
                self.WriteI2CData(offset, exitCommand)                    
                # Wait a second
                sleep(0.1 * attempts)            
    ...
    def SetMode(self, mode=enumModes.OPMode):
        ...
        try:
            ...
            currentMode = self.ReadI2CData(self.I2C_OP_HST_MODE, 5)[0]        
            ...
            self.WriteI2CData(self.I2C_OP_HST_MODE, mode)       
            
            # Wait for the mode change to complete
            newMode = self.ReadI2CData(self.I2C_OP_HST_MODE, 5)[0]        
