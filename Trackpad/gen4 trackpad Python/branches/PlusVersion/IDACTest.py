...
def IDACTest():
    ...
    configTest=configFile_data.get_item("Global IDAC",configPath)
    IDACTestLimits.GlobalIDACL=configTest.Min
    IDACTestLimits.GlobalIDACH=configTest.Max
#     configFile_data=xmlReader.captureXMLConfigFile()    
    configTest=configFile_data.get_item("Local IDAC",configPath)
    ...
    if ReCalibration:
        WaitForComplet=True
        bridge.WriteI2CData(0x24, 0x00, IDACCommands.ReCalMulCap)
        ...
        WaitForComplet=True
        bridge.WriteI2CData(0x24, 0x00, IDACCommands.ReCalSelCap)
        while (WaitForComplet):        
            sleep(0.2)
            datain=bridge.ReadI2CData(0x24, 0x00, 3)
            if (datain[2]&0x40==0x40):
    ...
    WaitForComplet=True
    bridge.WriteI2CData(0x24, 0x00, IDACCommands.ReadIDACs)
    while WaitForComplet:
        sleep(0.1)
        datain=bridge.ReadI2CData(0x24, 0x00, 3)        
