...
class BaselineCommands:
    ...
    GetSensorFullPanelScan     =[0xA0, 0x00, 0x0C, 0x00, 0x00, 0x01, 0xE5, 0x01]
...
#         return jsonReturn
    ...
    if ReCalibration:
        WaitForComplet=True
        bridge.WriteI2CData(0x24, 0x00, BaselineCommands.ReCalMulCap)
        ...
        WaitForComplet=True
        bridge.WriteI2CData(0x24, 0x00, BaselineCommands.ReCalSelCap)
        while (WaitForComplet):        
            sleep(0.2)
            datain=bridge.ReadI2CData(0x24, 0x00, 3)
            if (datain[2]&0x40==0x40):
    ...
    WaitForComplet=True
    bridge.WriteI2CData(0x24, 0x00, BaselineCommands.InitBaseline)
    while WaitForComplet:
        sleep(0.1)
        datain=bridge.ReadI2CData(0x24, 0x00, 3)        
    ...
    for i in range(readCount):        
        ...
        WaitForComplet=True
        bridge.WriteI2CData(0x24, 0x00, BaselineCommands.FullPanelScan)
        while WaitForComplet:
            sleep(0.1)
            datain=bridge.ReadI2CData(0x24, 0x00, 3)        
            if (datain[2]&0x80==0x80) or (datain[2]&0x40==0x40):
        ...
        WaitForComplet=True
        bridge.WriteI2CData(0x24, 0x00, BaselineCommands.GetSensorFullPanelScan)
        while WaitForComplet:
            sleep(0.1)
            datain=bridge.ReadI2CData(0x24, 0x00, 3)        
            if (datain[2]&0x80==0x80) or (datain[2]&0x40==0x40):
        ...
        BaselineTemp.append(datain[8:(8+TrackpadColume*TrackpadRow)])
        
    
    BaselineTemp=np.array(BaselineTemp,dtype='b')
    Baseline=BaselineTemp.mean(axis=0)
    #print Baseline
    
    
    for dataTemp in Baseline:
        returnData['Data'].append(int(dataTemp))
    
    for dataTemp in returnData['Data']:        
        if  dataTemp<BaselineLimits.BaselineLow:
            returnData['ErrorMessage']="Low Baseline"
            returnData['TestResult']='Fail'
            ...
            print jsonReturn
            return jsonReturn   
        if  dataTemp>BaselineLimits.BaselineHigh:
            returnData['ErrorMessage']="High Finger Signal"
            returnData['TestResult']='Fail'
...
if __name__=="__main__":      
    ...
    configTest=configFile_data.get_item("Baseline",configPath)    
    BaselineLimits.BaselineHigh=configTest.Max
    BaselineLimits.BaselineLow=configTest.Min
    readCount=configTest.Samples 
    BaselineCommands.GetSensorFullPanelScan[5]=TrackpadColume*TrackpadRow/256
    BaselineCommands.GetSensorFullPanelScan[6]=TrackpadColume*TrackpadRow%256
